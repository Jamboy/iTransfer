package com.lion.itransfer.ui.scan.custom;
...
import com.lion.itransfer.R;
import com.lion.itransfer.ui.scan.been.Info;
...
public class RadarViewGroup extends ViewGroup implements RadarView.IScanningListener {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            float x = (float) Math.cos(Math.toRadians(scanAngleList.get(i - 1) - 5))
                    * ((CircleView) child).getProportion() * mWidth / 2;
            if (x > getMeasuredWidth() - child.getMeasuredWidth()) {
                x = getMeasuredWidth() - child.getMeasuredWidth();
            }
            float y = (float) Math.sin(Math.toRadians(scanAngleList.get(i - 1) - 5))
                    * ((CircleView) child).getProportion() * mWidth / 2;
            if (y > getMeasuredHeight() - child.getMeasuredHeight()) {
                y = getMeasuredHeight() - child.getMeasuredHeight();
            }
            ((CircleView) child).setDisX(x);
            ((CircleView) child).setDisY(y);
    ...
    public void setDatas(SparseArray<Info> mDatas) {
        ...
        for (int j = 0; j < dataLength; j++) {
            ...
            if (map.keySet().contains(item.getName())) {
                scanAngleList.put(j, map.get(item.getName()));
            } else {
                Float angle = (float) (Math.random() * 360);
                map.put(item.getName(), angle);
                scanAngleList.put(j, angle);
            }
        }
        ...
        for (int i = 0; i < dataLength; i++) {
            ...
            circleView.setPhoneName(mDatas.get(i).getName());
            addView(circleView);
